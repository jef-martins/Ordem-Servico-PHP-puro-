...
if ( json_last_error() === JSON_ERROR_NONE ) {
    ...
    if ( $codigo && $descricao && $status && $garantia ) {

        $produto = new Produto();
        $ultimoID = $produto->inserirProduto( $codigo, $descricao, $status, $garantia );
        $ultimoProduto = $produto->retornarProduto( $ultimoID );
        echo Response::json( 201, 'success', $ultimoProduto );
    } else {
